...
logPath = "/opt/af-logs/index_nikkei.txt"
...
### Declare Dag
idxNikkeiDag = DAG('Index_Nikkei', description='Daily Index Stock - Nikkei 225', schedule_interval='0 8 * * *'  ,start_date =datetime(2021,10,31), catchup=False)

dagFunc_logInit = PythonOperator(task_id = 'InitLogging', python_callable=write_log_init, op_kwargs={'file_path': logPath}, dag=idxNikkeiDag)
dagFunc_execute = PythonOperator(task_id = 'Execution', python_callable=executeIndexApi, dag=idxNikkeiDag)
dagFunc_logEnd = PythonOperator(task_id = 'EndLogging', python_callable=write_log_end, op_kwargs={'file_path': logPath}, dag=idxNikkeiDag)
