...
logPath = "/opt/af-logs/crypt_bnb.txt"
...
### Declare Dag
cryptoBnbDag = DAG('Crypto_Binance', description='Daily Crypto Currency - BNB', schedule_interval='20 7 * * *'  ,start_date =datetime(2021,11,15), catchup=False)

dagFunc_logInit = PythonOperator(task_id = 'InitLogging', python_callable=write_log_init, op_kwargs={'file_path': logPath}, dag=cryptoBnbDag)
dagFunc_execute = PythonOperator(task_id = 'Execution', python_callable=executeIndexApi, dag=cryptoBnbDag)
dagFunc_logEnd = PythonOperator(task_id = 'EndLogging', python_callable=write_log_end, op_kwargs={'file_path': logPath}, dag=cryptoBnbDag)
