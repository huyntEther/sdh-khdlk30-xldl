...
logPath = "/opt/af-logs/crypt_eth.txt"
...
### Declare Dag
cryptoEthDag = DAG('Crypto_Ethereum', description='Daily Crypto Currency - ETH', schedule_interval='10 7 * * *'  ,start_date =datetime(2021,11,15), catchup=False)

dagFunc_logInit = PythonOperator(task_id = 'InitLogging', python_callable=write_log_init, op_kwargs={'file_path': logPath}, dag=cryptoEthDag)
dagFunc_execute = PythonOperator(task_id = 'Execution', python_callable=executeIndexApi, dag=cryptoEthDag)
dagFunc_logEnd = PythonOperator(task_id = 'EndLogging', python_callable=write_log_end, op_kwargs={'file_path': logPath}, dag=cryptoEthDag)
