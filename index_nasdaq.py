...
logPath = "/opt/af-logs/index_nasdaq.txt"
...
### Declare Dag
idxNasdaqDag = DAG('Index_Nasdaq', description='Daily Index Stock - NASDAQ', schedule_interval='0 22 * * *'  ,start_date =datetime(2021,11,1), catchup=False)

dagFunc_logInit = PythonOperator(task_id = 'InitLogging', python_callable=write_log_init, op_kwargs={'file_path': logPath}, dag=idxNasdaqDag)
dagFunc_execute = PythonOperator(task_id = 'Execution', python_callable=executeIndexApi, dag=idxNasdaqDag)
dagFunc_logEnd = PythonOperator(task_id = 'EndLogging', python_callable=write_log_end, op_kwargs={'file_path': logPath}, dag=idxNasdaqDag)
