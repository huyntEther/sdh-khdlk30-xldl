...
logPath = "/opt/af-logs/index_dowjones.txt"
...
### Declare Dag
idxDowjonesDag = DAG('Index_DowJones', description='Daily Index Stock - Dow Jones', schedule_interval='0 21 * * *'  ,start_date =datetime(2021,11,1), catchup=False)

dagFunc_logInit = PythonOperator(task_id = 'InitLogging', python_callable=write_log_init, op_kwargs={'file_path': logPath}, dag=idxDowjonesDag)
dagFunc_execute = PythonOperator(task_id = 'Execution', python_callable=executeIndexApi, dag=idxDowjonesDag)
dagFunc_logEnd = PythonOperator(task_id = 'EndLogging', python_callable=write_log_end, op_kwargs={'file_path': logPath}, dag=idxDowjonesDag)
