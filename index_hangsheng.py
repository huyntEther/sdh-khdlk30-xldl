...
logPath = "/opt/af-logs/index_hangseng.txt"
...
### Declare Dag
idxHangseng100Dag = DAG('Index_HangSeng', description='Daily Index Stock - Hang Seng Index', schedule_interval='0 9 * * *'  ,start_date =datetime(2021,11,1), catchup=False)

dagFunc_logInit = PythonOperator(task_id = 'InitLogging', python_callable=write_log_init, op_kwargs={'file_path': logPath}, dag=idxHangseng100Dag)
dagFunc_execute = PythonOperator(task_id = 'Execution', python_callable=executeIndexApi, dag=idxHangseng100Dag)
dagFunc_logEnd = PythonOperator(task_id = 'EndLogging', python_callable=write_log_end, op_kwargs={'file_path': logPath}, dag=idxHangseng100Dag)
